...
function drawFamilyTree(treeData) {
  ...
  // === Wait for images, then adjust row spacing by actual heights ===
  function adjustRowSpacingOnceImagesReady() {
    ...
    const performAdjustment = () => {
      ...
      // bucket cards by initial top (generation)
      const rows = {};
      cards.forEach(card => {
      ...
      // reposition rows cumulatively by tallest card in each row
      let cumulativeY = 0;
      const spacing = 40;
      ...
      // after spacing is final, draw connectors using actual DOM positions
      redrawConnectors();
      centerSubject();
    };
  ...
  // === Draw connectors based on real DOM positions ===
  ...
  // === Center subject after everything is laid out ===
  ...
  // kick off image-aware adjustment (this will call redraw + center when done)
